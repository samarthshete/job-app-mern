...
const initialState = {
  ...
  jobType: "full-time",
  statusOptions: ["applied", "interviewing", "declined", "pending", "offer"],
  status: "pending",
  jobs: [],
  totalJobs: 0,
  numOfPages: 1,
  ...
  totalContacts: 0,
  tag: "recruiter",
  tagOptions: ["recruiter", "candidate", "colleague", "employee"],
  firstName: "",
  lastName: "",
  phoneNumber: "",
  companyName: "",
...
const AppProvider = ({ children }) => {
  ...
  const createJob = async () => {
    ...
    try {
      const { position, company, jobLocation, salary, jobType, status } = state;
  ...
  const createContact = async () => {
    ...
    try {
      const { tag, firstName, lastName, companyName, phoneNumber, email } =
        state;
      await authFetch.post("/contacts", {
        tag,
        firstName,
        lastName,
        companyName,
  ...
  const getContacts = async () => {
    ...
    try {
      ...
      const { Contact, totalContacts } = data;
      dispatch({
